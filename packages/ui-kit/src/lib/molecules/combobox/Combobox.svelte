<script lang="ts">
	...
	function removeItem(toRemoveItem: string | Record<string, any>) {
		const index = items.findIndex((item) => {
			if (typeof item === 'string' && typeof toRemoveItem === 'string')
...
<div class="w-full rounded-lg" use:clickOutside={() => (showDropdown = false)}>
	...
	>
		{#if icon}
			<span
				class="text-highlight/80 absolute inset-y-0 left-0 flex items-center justify-center border-primary px-2"
			>
	...
	{#if showDropdown}
		<div
			class={`absolute left-0 right-0 top-full z-1000 mt-1 w-full min-w-60 cursor-pointer bg-primary p-2 text-sm font-normal shadow-2xl ${hideInput ? 'w-60' : ''}`}
		>
	...
	<div class="flex h-10 items-center gap-2">
		<div class="min-w-0 flex-1">
			<div class="flex h-10 flex-wrap items-center gap-2 overflow-hidden">
				<span
					class="inline-flex max-w-full items-center gap-2 rounded-lg border bg-white px-2.5 py-1 text-xs"
				>
					<span class="max-w-[20rem] truncate font-medium"
						>{chipItems[0].name || chipItems[0].label}</span
					>
					<button
						type="button"
		...
		{#if rest.length}
			<div class="relative shrink-0">
				...
				{#if openFlag}
					<div
						class="absolute right-0 top-full z-2000 mt-2 max-h-48 w-72 overflow-auto rounded-xl border bg-white p-2 shadow"
					>
